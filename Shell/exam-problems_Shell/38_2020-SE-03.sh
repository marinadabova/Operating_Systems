...
version=$(cat "$P2}/version")
archive=$(tar cJf - -C "${2}/tree" .)
sum=$(echo "${archive}" | sha256sum | awk '{print $1}')

archive_filename="${sum}.tar.xz"
echo "${archive}" | xz > "${archive_filename}"

existing_entry=$(grep "^${package_name}-${version} " "${1}/db")

if [[ -n "${existing_entry}" ]]; then
    sed -i "/^${package_name}-${version} /d" "${1}/db"
fi

echo "${package_name}-${version} ${sum}" >> "${1}/db"
sort -o "${1}/db" "${1}/db"

mv "${archive_filename}" "${1}/packages"
