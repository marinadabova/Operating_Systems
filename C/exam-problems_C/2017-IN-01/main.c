...
int  main(int argc, char* argv[]){
	if(argc != 5){
		errx(1,"We need four arguments");
	}
	int fd1=open(argv[1], O_RDONLY);
	if(fd1 < 0){
		err(2,"Error while openning");
...
/*Зад. 61 2017-IN-01 Напишете програма на C, която приема четири параметъра – имена на двоични
файлове.
Примерно извикване:
$ ./main f1.dat f1.idx f2.dat f2.idx
Първите два (f1.dat и f1.idx) и вторите два (f2.dat и f2.idx) файла са входен и изходен комп-
лект със следния смисъл:
